		...
		If ($checkNormal=True:C214)  // & ($property#wk background color)  // YES
			...
			If ($source[$property]#Null:C1517)  // does the property exist in "Source" ?
				If ($normal[$property]#Null:C1517)  // does the property exist in "Normal" ?
					...
					Case of 
						: ($type=Is real:K8:4) | ($type=Is text:K8:3)  // let's compare text and numeric
				...
				Else 
					  // Should never happend, "Normal" MUST have this property
					  // IF not present, can't compare, so DO NOTHING
					
					
					  // Finaly yes, it may hapend with background image
